...
jQuery.Fotorama = function ($fotorama, opts) {
  ...
  that.resize = function (options) {
    ...
    var time = arguments[1] || 0,
        ...
        if (o_nav) {
            var nav_height = $nav.css('position') != 'absolute' && $nav.css('position') != 'fixed' ? $nav.outerHeight() : 0;
        }
        
        var $fotorama_holder   = $wrap.parent().parent();
        var temp_window_height = $fotorama_holder.css('position') == 'absolute' || $fotorama_holder.css('position') == 'fixed' ? $fotorama_holder.height() : $WINDOW.height();
        
        var  windowHeight = temp_window_height - (o_nav ? nav_height : 0);
