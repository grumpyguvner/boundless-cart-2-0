...
class Theme_Catalog_RenderSlotPlugin extends TB_ExtensionPlugin
{
    ...
    public function getSlotContents($slot, array $data = array(), $route = null)
    {
        ...
        if ($themeData->route != $route) {
            $keys = $this->extension->getSetting('area_keys');
            $area_name = 'content';
            $result = array();
        ...
        $widget = $this->manager->getWidgetManager()->createAndFilterWidgetFromId($widget_id, $widget_settings);
    ...
    protected function getWidgetData($request_areas, $widget_id, $partial = false)
    {
        ...
        $keys = $this->manager->getWidgetManager()->loadAreasData($area_keys);
        ...
        foreach ($request_areas as $area_name => $area_id) {
            ...
            $area_data = $this->manager->getWidgetManager()->getAreaSettings($area_name . '_' . $area_id, 'area');
