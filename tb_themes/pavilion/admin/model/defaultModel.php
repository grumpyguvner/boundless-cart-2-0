...
class Theme_Admin_DefaultModel extends Theme_DefaultModel
{
    ...
    public function getAllStores()
    {
        ...
        $stores = array_merge(array(
            0 => array(
                'store_id' => 0,
                'name'     => $this->manager->getDbSettingsHelper()->getKey('config_name', 0, 'config') . ' (Default)'
            )
        ), $this->getOcModel('setting/store')->getStores());

        foreach ($stores as &$store) {
            $store['has_theme'] = $this->manager->getDbSettingsHelper()->getKey('config_template', $store['store_id'], 'config') == $this->manager->getThemeMachineName();
        }
