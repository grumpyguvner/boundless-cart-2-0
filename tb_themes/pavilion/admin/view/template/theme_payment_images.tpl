...
<div class="tb_subpanel">
  ...
  <div class="s_sortable_holder tb_style_1 tbPaymentImagesContainer"><?php $i = 0; foreach ($images as $image): ?><div class="s_sortable_row tbImageRow">
      ...
      <div class="s_row_1">
        <label>Type</label>
        <div class="s_select">
          <select name="payment_images[rows][<?php echo $i; ?>][type]">
      ...
      <div class="s_row_1 tbImageSealCode">
        <label>Code</label>
        <textarea name="payment_images[rows][<?php echo $i; ?>][seal_code]"><?php echo $image['seal_code']; ?></textarea>
      </div>
      <div class="s_row_1 tbImageDialog">
        <label>Image</label>
        <input type="hidden" name="payment_images[rows][<?php echo $i; ?>][file]" value="<?php echo $image['file']; ?>" id="payment_images_file_<?php echo $i; ?>" />
        <span class="tb_thumb">
      ...
      <div class="s_row_1 tbImageUrl">
        <label>Link</label>
        <input type="text" name="payment_images[rows][<?php echo $i; ?>][link_url]" value="<?php echo $image['link_url']; ?>" />
        <span class="s_metric">
          <select class="s_metric" name="payment_images[rows][<?php echo $i; ?>][link_target]">
  ...
  <script type="text/javascript">
    ...
    var rows_num = $container.find(".tbImageRow").length;
    ...
    $container.on("change", 'select[name$="[type]"]', function() {
      ...
      $row.find('.tbImageDialog, .tbImageUrl').toggle($(this).val() == 'image');
      $row.find('.tbImageHtml').toggle($(this).val() == 'html');
  ...
  <script type="text/template" id="payment_images_template">
    <div class="s_sortable_row tbImageRow">
      ...
      <div class="s_sortable_contents">
        ...
        <div class="s_row_1 tbImageSealCode">
          <label>Code</label>
          <textarea name="payment_images[rows][{{row_num}}][seal_code]"></textarea>
        </div>
        <div class="s_row_1 tbImageDialog">
          <label>Image</label>
