...
<div class="s_builder_new_row clearfix">
  ...
  <?php if ($can_delete || $tbManager->getThemeConfig('admin_show_copy_area') && $stores): ?>
  <div class="right">
    <?php if ($tbManager->getThemeConfig('admin_show_copy_area') && $stores): ?>
    <label>Copy to:</label>
    <select>
    ...
    <?php endif; ?>
    <?php if ($can_delete): ?>
      <a class="s_button s_h_30 s_white s_icon_10 s_delete_10 tbDeleteArea" href="<?php echo $tbUrl->generate('layoutBuilder/removeSettings', array('area_type' => $area_type, 'area_id' => $area_id, 'area_name' => $area_name, 'current_area_id' => $area_id, 'current_area_type' => $area_type, 'record_type' => 'rows')); ?>">Remove scope</a>
    <?php endif; ?>
  </div>
  <?php endif; ?>
</div>
