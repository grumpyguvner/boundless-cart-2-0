...
tbApp.bootAdmin = function() {
    ...
    var initTabs = function($selector, cookie_name) {
        ...
        $selector.tabs({
            ...
            active: tbApp.cookie.get(cookie_name, 0),
    ...
    initTabs($("#tb_cp_content_wrap"),         "tbMenuLevel1");
    initTabs($("#tb_cp_panel_theme_settings"), "tbSettingsMenu");
    initTabs($("#tb_cp_panel_extensions"),     "tbExtensionsMenu");
    ...
    $("#tb_cp_content").find("> div.s_server_msg a.s_close").bind("click", function() {
    ...
    var theTimeout = setTimeout(function() {
        $tb_cp_wrap.find("div.success").hide("slow");
    }, 5000);
...
function assignColorPicker($element, show_transparent, lazy) {
    ...
    $element.next("input").bind("keydown", function(e) {
        ...
        if ((e.shiftKey && /[0-9]$/i.test(String.fromCharCode(e.keyCode))) || !(e.ctrlKey && e.keyCode == 86) && $.inArray(e.keyCode, [8,17,46]) < 0  && !/[0-9a-fA-F]$/i.test(String.fromCharCode(e.keyCode))) {
            e.preventDefault();
        }
