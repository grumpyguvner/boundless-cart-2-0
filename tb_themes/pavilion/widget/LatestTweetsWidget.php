...
class Theme_LatestTweetsWidget extends TB_Widget
{
    ...
    public function render(array $view_data = array())
    {
        if (isset($this->themeData['twitter'][$this->language_id])) {
            $settings_lang = $this->themeData['twitter'][$this->language_id];
        } else {
            foreach ($this->themeData['twitter'] as $settings_lang) {
                break;
            }
        }

        if ($this->settings['display_type'] == 'default') {
            $view_data['twitter_code'] = $settings_lang['code'];
        } else {
            $cache_key = $this->getId() . '.' . (TB_RequestHelper::isRequestHTTPS() ? 'https.': '') . $this->language_id;
            $get_remote_content = true;

            if ($tweets = $this->manager->getCacheVar($cache_key)) {
            ...
            if ($get_remote_content) {
                $view_data['tweets'] = $this->getTweets($settings_lang);
                if ($this->themeData['system']['cache_enabled'] && isset($this->themeData['system']['cache_widgets']['Theme_LatestTweetsWidget']['ttl'])) {
                    $this->manager->setCacheVar($cache_key, $view_data['tweets'], (int) $this->themeData['system']['cache_widgets']['Theme_LatestTweetsWidget']['ttl'] * 60);
                }
            }

            $view_data['twitter_username'] = $settings_lang['username'];
        }
    ...
    function timeElapsedString($timestamp){
        ...
        $trans = $tbLang = $this->manager->getThemeExtension()->loadDefaultTranslation();
