...
class TB_AdminController extends TB_Controller
{
    ...
    public function __construct(TB_ThemeManager $manager, TB_AdminExtension $extension)
    {
    ...
    {
        if ($key = $this->manager->getOcConfig()->get($this->context->getBasename() . '_livePreviewToken')) {
            $parts = explode('!*!', $key);
            if (count($parts) != 3) {
    ...
    public function getSectionAttributes($section, $id)
    {
        ...
        if ($this->hasAjax($section) && $this->manager->getThemeConfig('admin_ajax')) {
            $result = 'href="' . $href . '"';
        } else {
    ...
    public function renderSection($section = null, $data = array())
    {
        ...
        if  (null == $section && $this->manager->getThemeConfig('admin_ajax')) {
        ...
        if (!isset($this->section_map[$section]) || ($this->section_map[$section]['ajax']) && $this->manager->getThemeConfig('admin_ajax')) {
            return;
        }
    ...
    protected function initDataVars()
    {
        ...
        $this->data['languages']       = $this->manager->getEnabledLanguages();
        $this->data['stores']          = $this->getOcModel('setting/store')->getStores();
        $this->data['all_stores']      = $this->manager->getThemeModel()->getAllStores();
        $this->data['no_image']        = $this->getOcModel('tool/image')->resize('no_image.jpg', 100, 100);
        $this->data['tbUrl']           = $this->tbUrl;
        $this->data['token']           = $this->session->data['token'];
        $this->data['show_extensions'] = count($this->manager->getExtensions()) - count($this->manager->getCoreExtensions()) > 0;
    }
