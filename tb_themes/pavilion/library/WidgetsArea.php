...
class TB_WidgetsArea
{
            ...
            foreach ($row['columns'] as &$column) {
                if (isset($column['widgets'])) {
                    foreach ($column['widgets'] as $key => $value) {
                        $widget = $this->widgetManager->createAndFilterWidgetFromId($value['id'], $value['settings']);
                        if ($widget->isActive()) {
                            $this->addWidget($widget);
                            if ($widget instanceof Theme_GroupWidget && isset($value['subwidgets'])) {
                                foreach ($value['subwidgets'] as $vvalue) {
                                    $subWidget = $this->widgetManager->createAndFilterWidgetFromId($vvalue['id'], $vvalue['settings']);
    ...
    {
        $this->widgetManager = TB_StaticThemeManager::get()->getWidgetManager();
