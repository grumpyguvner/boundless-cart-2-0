...
class TB_StyleBuilder
{
    ...
    protected function processColorRule(array $color_rule, $id = null, $bg_solid_color = null)
    {
        ...
        if (null !== $id && $id != 'header' && $id != 'main_navigation_row') {
            if (strpos($id, 'GalleryWidget') !== false) {
                $scope = '[id*="' . $id . '"]';
            }
            else {
                $scope = '#' . $id;
        ...
        // Fixed rules if body color is changed
        if ((empty($color_rule['elements']) && null !== $id) || $color_rule['elements'] == '#header') {
            $color_rule['output'] .= $scope . " .tb_bg_str_1, " . $scope . " .tb_bg_hover_str_1:hover, " . $scope . " .tb_widget_text blockquote { background-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.05); }\n";
            $color_rule['output'] .= $scope . " .tb_bg_str_2, " . $scope . " .tb_bg_hover_str_2:hover, " . $scope . " .tb_table_1 thead > * { background-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.1);  }\n";
            $color_rule['output'] .= $scope . " .tb_bg_str_3, " . $scope . " .tb_bg_hover_str_3:hover { background-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.2); }\n";
            $color_rule['output'] .= $scope . " .tb_bg_str_4, " . $scope . " .tb_bg_hover_str_4:hover { background-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.3); }\n";
            ...
            $color_rule['output'] .= $scope . " .tb_text_str_5, " . $scope . " .tb_text_hover_str_5:hover, " . $scope . " .tb_widget_text blockquote { color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.8) !important; }\n";
            $color_rule['output'] .= $scope . " .tb_border_str_1 { border-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.1); }\n";
            $color_rule['output'] .= $scope . " .tb_border_str_2 { border-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.2); }\n";
            $color_rule['output'] .= $scope . " .tb_border_str_3 { border-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.4); }\n";
            $color_rule['output'] .= $scope . " .tb_border_str_4 { border-color: " . 'rgba(' . TB_Utils::hex2rgb($color_rule['color'], true) . ", 0.6); }\n\n";
        }
    ...
    public function addFonts(array $fonts, $scope_id = '')
    {
        foreach ($fonts as $key => $font) {
            if (empty($font['type']) || $font['family'] == 'inherit') {
                continue;
            }
    ...
    {
        return $this->baseline_height + ceil((2*$font_size / $this->baseline_height) - 2) * ($this->baseline_height/2);
    }
    ...
    public function getWebFonts()
    {
        ...
        $combine = $this->context->getThemeConfig('catalog_merge_google_fonts');
    ...
    protected function combineFonts()
    {
        ...
        foreach ($this->fonts as $section => $font) {

            if ($font['type'] != 'google') {
                $css_weight = 'default';
                $css_style = 'default';

                if ($font['type'] == 'built' && isset($font['variant'])) {
                    $css_weight = 'normal';
    ...
    {
        if (!isset($settings['border']) && !isset($settings['border_radius'])) {
            return;
        }
        ...
        if (isset($settings['border'])) {
            ...
            foreach (array('top', 'right', 'bottom', 'left') as $side) {
                if (isset($border[$side]) && $border[$side]['width'] > 0) {
                    ...
                    $border_css = 'border-' . $side . ': ' . $border[$side]['width'] . 'px ' . $border[$side]['style'] . ' ' . $color . ';';
                    $result .= $border_css;
                }
            }
        }
        ...
        if (isset($settings['border_radius']) && !empty($settings['border_radius'])) {
            ...
            if ($empty_positions < 4) {
                $result .=  'border-radius: ' . $radius['top_left'] . 'px ' . $radius['top_right'] . 'px ' . $radius['bottom_right'] . 'px ' . $radius['bottom_left'] . 'px;' . "\n";
            }
        }
    ...
    public function buildBackgroundCss($settings, $scope)
    {
        ...
        $this->addCss($css_content, $scope . ' > .tb_wrap');
    }
