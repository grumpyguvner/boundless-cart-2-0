  ...
  function mightySlider(frame, options, callbackMap) {
    ...
    function scaleSlider() {
      ...
      if ($parent.hasClass('fullscreen')) {
        var scene_width  = $parent.find('.ms_scene').first().width();
        var scene_height = $parent.find('.ms_scene').first().height();
        var ratio = (scene_width >= scene_height ? scene_width : scene_height) / autoScale.width;
      }
      else {
        var ratio = $parent.width() / autoScale.width;
      }
...
function fireslider_fullwidth_resize($slider, responsive_width) {
    var slider_width = $($slider.frame).width();

    if (slider_width <= responsive_width) {
        if ($slider.options.autoScale != 1) {
            $slider.options.autoScale = 1;
            $slider.reload()
        }
    } else {
        if ($slider.options.autoScale == 1) {
            $slider.options.autoScale = 0;
            $slider.reload()
        }
    }
}

function fireslider_fullscreen_resize($slider, $slider_dom, responsive_width, responsive_height, offset_top, offset_left) {
    var ratio          = responsive_width / responsive_height;
    var current_width  = $slider_dom.width();
    var current_height = $slider_dom.height();
    var current_ratio  = current_width / current_height;
    if (current_width <= responsive_width || current_height <= responsive_height) {
        if (ratio > current_ratio) {
            var new_width  = current_width;
            var new_height = Math.ceil((current_width * responsive_height) / responsive_width)
        } else {
            var new_height = current_height;
            var new_width  = Math.ceil((new_height * responsive_width) / responsive_height)
        }
        $slider_dom.find(".ms_scene").css("width", (new_width - offset_left * 2)).css("height", (new_height - offset_top * 2)).css("marginTop", -((new_height / 2) - offset_top)).css("marginLeft", -((new_width / 2) - offset_left))
